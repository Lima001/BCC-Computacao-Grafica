...
const char* fragmentShaderSource2 = R"(
    ...
    #define B_COLOR vec4(0.2,0.2,0.3,1.0)       // Background color
    #define P_COLOR vec4(1.0,0.0,0.0,1.0)       // Bézier curve control point color
    #define L_COLOR vec4(0.7,0.7,0.7,1.0)       // Bézier curve line color
    #define L_WIDTH 1.0                         // Line thickness
    #define P_WIDTH 1.0                         // Point radius
    ...
    // Inverse transformation matrix applied to graphics. It's the inverse of 
    ...
    vec3 getWorldCoord(){
        // Return the coresponding world coordinate of the current fragment coordinate.
        // To accomplish this, just undo the vertex shader processing.
        
        vec4 viewport = vec4(0.0,0.0,800.0,600.0);
        
        // NDC coordinate of the current fragment
        vec4 ndcPos;
        ndcPos.xy = ((2.0 * gl_FragCoord.xy) - (2.0 * viewport.xy)) / (viewport.zw) - 1;
        ndcPos.z = (2.0 * gl_FragCoord.z - gl_DepthRange.near - gl_DepthRange.far) / (gl_DepthRange.far - gl_DepthRange.near);
        ndcPos.w = 1.0;

        // Undo cliping and graphic transformations
        vec4 clipPos = ndcPos / gl_FragCoord.w;
        vec4 worldCoord = inverseMat * clipPos;

        // Current fragment's world coordinate
        vec3 s = vec3(worldCoord.x, worldCoord.y, worldCoord.z); 
        return s;
    }

    float distBezier(vec3 p){
        ...
        vec3 w = p0;
        vec3 x = -3*p0 + 3*p1;
        vec3 y = 3*p0 -6*p1 + 3*p2;
        vec3 z = -p0 + 3*p1 -3*p2 +p3;
        ...
        // solution)
        vec3 pB = w + x*t + y*t*t + z*t*t*t;

        return distance(p,pB);
    }

    void main(){
        // Obtain the world coordinate for the current fragment 
        vec3 p = getWorldCoord();
